...
function dragDrop() {
    ...
    $form.on('drag dragstart dragend dragover dragenter dragleave drop', function (e) {
        ...
        .on('click', '#clear-images', function (e) {
            ...
            $('.drop-image').fadeOut(400, function () {
                $(this).remove();
            });
        });
}
...
function addHotel() {
    ...
    $(addHotel).unbind('click').bind('click', function () {
        var containerFirst = $('.hotel-container').first();
        var containerLast = $('.hotel-container').last();
        var select = $(containerFirst).find('.new_hotel').last().find('select');
        var freeHotels = $(select).children('option');
        $(containerLast).append(hotel);
        var newSelect = $(containerLast).find('.new_hotel').last().find('select');
